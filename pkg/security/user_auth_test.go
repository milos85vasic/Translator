...
func TestUserAuth_AuthenticateUser(t *testing.T) {
	...
	// Hash password and add to store
	hashedPassword, err := auth.HashPassword(testUser.Password)
	require.NoError(t, err)
	testUser.Password = hashedPassword
	auth.AddUserToStore(testUser)
...
func TestUserAuth_RateLimiting(t *testing.T) {
	...
	t.Run("RateLimitRecovery", func(t *testing.T) {
		...
		auth.rateLimiter[user.Username] = &RateLimitState{
			Tokens:   auth.config.RateLimitRPS, // Refilled tokens
