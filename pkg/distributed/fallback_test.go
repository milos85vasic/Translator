...
func TestFallbackManager_PrimaryFails(t *testing.T) {
	...
	t.Run("PrimaryFailsFallbackSucceeds", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, "success", result.Status)
		assert.Equal(t, "http://fallback1:8080", result.Endpoint)
		assert.Greater(t, result.Attempt, 1)
	})
}
...
func TestFallbackManager_BackoffStrategy(t *testing.T) {
	...
	t.Run("BackoffStrategy", func(t *testing.T) {
		...
		// Should not have exceeded max backoff
...
func TestFallbackManager_TimeoutHandling(t *testing.T) {
	...
	t.Run("TimeoutHandling", func(t *testing.T) {
		...
		_, err := manager.ExecuteWithFallback("slow-request")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "timeout")
		
		// Should have failed within reasonable time
		duration := time.Since(start)
...
func (fm *FallbackManager) ExecuteWithFallback(request string) (*FallbackResult, error) {
...
func (fm *FallbackManager) ExecuteWithContextFallback(ctx context.Context, request string) (*FallbackResult, error) {
	var lastError error
...
func (fm *FallbackManager) ExecuteWithRetry(request string) (*FallbackResult, error) {
...
func (fm *FallbackManager) executeEndpoint(ctx context.Context, endpoint, request string) (*FallbackResult, error) {
	// Mock endpoint execution
	if endpoint == "http://primary:8080" {
		// Simulate primary failure for some tests
...
func (fm *FallbackManager) CheckEndpointHealth() map[string]bool {
